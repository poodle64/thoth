...
`--no-default-features` is required on Linux: the default features (Parakeet + FluidAudio) are off here. FluidAudio is Apple-Neural-Engine only (macOS/Apple Silicon). Parakeet — now the official k2-fsa `sherpa-onnx` crate — **does** build and link on Linux, and it links **statically** (no extra runtime library to ship); this was previously blocked when the backend used `sherpa-rs`, whose Linux package shipped no static archive (see [#53](https://github.com/poodle64/thoth/issues/53)). It is still left out of the default Linux build pending runtime verification on a glibc desktop. A headless smoke test exists — `cargo run --example parakeet_smoke --no-default-features --features vulkan,parakeet -- <model_dir> <audio.wav>` — and it currently **crashes at model init under the Nix toolchain** (`free(): invalid pointer`, before any audio is decoded; the prebuilt onnxruntime in `sherpa-onnx-sys` clashing with Nix's libstdc++/allocator). The shipped `.deb`/`.AppImage` build on Ubuntu, where this may not reproduce, but the runtime is unconfirmed there too, so Parakeet stays opt-in: add `--features parakeet` to test. If the build can't reach GitHub releases to download the prebuilt sherpa archive, point it at a local copy with `SHERPA_ONNX_LIB_DIR`. The default Linux transcription backend is Whisper (whisper.cpp).
