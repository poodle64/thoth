 ...
 * `<script>` block) before Svelte has extracted the CSS, so Tailwind's CSS parser
 * blows up on the first JS token — the `Invalid declaration: \`invoke\`` /
 * `getCurrentWindow` 500s seen in the dev log.
 *
