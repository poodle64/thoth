...
- The recording start tone now plays reliably (#58). Two faults: it was gated behind a model-readiness check the stop tone doesn't have (so a cold start could begin recording before the check passed and skip the tone), and — the deeper cause — it was played through NSSound, which shares the app's audio output and got clipped or silenced when the app opened the microphone to record on the first press after the ~45-second warm-stream teardown. Recording cues now play through the macOS System Sound server (`AudioServicesPlaySystemSound`), the same mechanism dictation itself uses; it runs in a separate process, so the cue is fully decoupled from recording and plays cleanly regardless of how recently you last recorded. The start tone is also no longer gated on model-readiness — it fires whenever a new recording begins, like the stop tone.
