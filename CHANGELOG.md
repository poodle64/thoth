...
- The recording start tone now plays reliably and without clipping (#58). Two faults, both only on the first press after the ~45-second warm-stream teardown: (1) it was gated behind a model-readiness check the stop tone doesn't have, so a cold start could begin recording before the check passed and the tone was skipped entirely — it now fires whenever a new recording begins, gated only on "not already recording", like the stop tone; (2) when it did play, the cold-start audio-device open happened ~80 ms into the half-second tone and CoreAudio's session reconfiguration clipped it ("cut in half"), so the device is now opened _before_ the tone rather than during it. Both are no-ops on a warm start, which always worked.
