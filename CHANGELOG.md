...
- The recording start tone now plays reliably (#58). It was gated behind a model-readiness check that the stop tone does not have, so after the ~45-second idle teardown a cold start could begin recording before the check passed and the tone was silently skipped — which is why it played when starting again quickly but not after a minute or two idle. The start tone now fires whenever a new recording begins, gated only on "not already recording", exactly like the stop tone.
